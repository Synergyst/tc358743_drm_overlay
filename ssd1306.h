...
// Pixel colors
#define SSD1306_PIXEL_OFF 0
#define SSD1306_PIXEL_ON  1

// Page-aligned raw write: write 'w' bytes to a single page starting at column x
uint8_t ssd1306_oled_write_raw(uint8_t x, uint8_t page, const uint8_t* buf, uint8_t w);

// Page-aligned blit: write page_count pages of width 'w' starting at (x, page_start)
// 'buf' layout: page 0 contains w bytes, then page 1, etc. (total w * page_count bytes)
uint8_t ssd1306_oled_blit_pages(uint8_t x, uint8_t page_start, uint8_t w, uint8_t page_count, const uint8_t* buf);

// Lightweight framebuffer API (1bpp, page layout: 8 vertical pixels per byte)
...
// Draw a 1bpp row-major bitmap at (x,y). 'w' x 'h' in pixels. 'src_stride_bytes' is bytes per source row.
// Source bits are MSB-first in each byte (common for 1bpp bitmaps).
...
// Flush only a rectangle to display (clipped internally)
